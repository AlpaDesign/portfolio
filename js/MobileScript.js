$(function(){
	...
	sliderLi.find("li").click(function(){
		moving = false;
	});
	sliderLi.find("li").on("touchstart", function(e){
		var evt=e.originalEvent;
		xDown=evt.touches[0].clientX;
		yDown=evt.touches[0].clientY;
		direction= "";
	});
    sliderLi.find("li").on("touchmove", function(e){
        var evt=e.originalEvent;
        if(sliderLi.is(":animated"))return;
        if(moving == true) return;
        
        direction = sliderFunction(evt, xDown, yDown);
        pos = sliderLiMove;
		if(direction == "right"){
			if(dragn > 0){
				dragn-=1;
    ...
    sliderLi.find("li").on("touchend", function(e){
        if(direction == "right"){
           moveRight(dragn, pos);
		}
		else if(direction == "left"){
	...
	function moveRight(n, pos){
		...
		sliderLi.animate({left: 0},500, function(e){
			moving = false;
		});
        sliderLi.find("li").removeClass("active");
